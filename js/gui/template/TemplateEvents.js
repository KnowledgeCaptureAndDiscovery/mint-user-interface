...
import { GraphLinkConfig, GraphVariableConfig, GraphPreviewVariableConfig } from "./config/GraphItemConfig.js";

export function TemplateEvents(template, canvas) {
...
TemplateEvents.prototype.enableSelectEvent = function() {
  ...
  var drag = d3.drag()
    .on("start", function(d) {
      // Start selection box
      me.selectbox.startx = d3.event.x/me.template.scale;
      me.selectbox.starty = d3.event.y/me.template.scale;
      me.selectbox
    ...
    .on("drag", function(d) {
      ...
      var x = d3.event.x/me.template.scale;
      var y = d3.event.y/me.template.scale;
...
TemplateEvents.prototype.enableEventsForItem = function(gitem) {
  ...
  drag.on("start", function(d) {
    // Select node variables on dblclick
    var e = d3.event.sourceEvent;
    var t2 = e.timeStamp;
    var t1 = me.last_tap || t2;
    var dt = t2 - t1;
    me.last_tap = t2;
    if (dt && dt < 400) {
      // On a double-click/tap
      if (gitem.inputLinks) {
...
TemplateEvents.prototype.initializeDraggerItems = function() {
  if (!this.tmplink) {
    this.tmplink = new GraphLink(this.template.graph, "__dragger_link",
      null, null, null, null, null, [], //this.template.graphItems,
      new GraphLinkConfig());
  }
...
TemplateEvents.prototype.linkDrag = function() {
  ...
  // Set coordinates of temporary port if dragged from a variable
    ...
    this.tmpport.setCoords({
      x: x,
      y: y + this.tmpvariable.config.portsize / 2
    });
  }
