...
$_documentContainer.innerHTML = `<dom-module id="mint-tabs-overlay">
...
class MintTabsOverlay extends PolymerElement {

  static get is() { return 'mint-tabs-overlay'; }
  ...
  _targetChanged(newTarget, oldTarget) {
    if (!this._transitionsInFlight.length) {
      if (this._lastTarget) {
        this._lastTarget.classList.remove('mint-tabs-overlay-static-above');
      }
  ...
  _moveComplete() {
    if (this._lastTarget !== this.target) {
      ...
      if (this._lastTarget) {
        this._lastTarget.classList.add('mint-tabs-overlay-static-above');
      }
...
customElements.define(MintTabsOverlay.is, MintTabsOverlay);
